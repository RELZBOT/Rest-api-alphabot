...
const listkey = ["Relzbot", "relzmodz"];
...
router.get('/addapikey', (req, res, next) => {
    ...
    if (apikey != 'Relzbot') return res.json(loghandler.invalidKey)
...
router.get('/cewe/vietnam', async (req, res, next) => {
        var apikeyInput = req.query.apikey
	if(!apikeyInput) return res.json(loghandler.notparam)	
	if (apikeyInput != 'Relzbot')  return res.json(loghandler.invalidKey)
...
router.get('/cewe/malaysia', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)	
	if (apikeyInput != 'Relzbot')  return res.json(loghandler.invalidKey)
       fetch(encodeURI(`https://raw.githubusercontent.com/relzzofc/Asupan/main/cecan/malaysia.json`))
        .then(response => response.json())
        .then(data => {
        var result = data;
        var result = data[Math.floor(Math.random() * data.length)];
